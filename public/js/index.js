...
function active(id) { //修改对应id的背景图片地址。
  switch (id) {
    case 'delicacy':
      getID('iphone_img').style.background = 'url(image/delicacy.png) 0 0/contain';
      addClass('active', getID('delicacy'));
      index = 0;
      break;
    case 'desserts':
      getID('iphone_img').style.background = 'url(image/desserts.png) 0 0/contain';
      addClass('active', getID('desserts'));
      index = 1;
      break;
    case 'fresh':
      getID('iphone_img').style.background = 'url(image/fresh.png) 0 0/contain';
      addClass('active', getID('fresh'));
      index = 2;
      break;
    case 'supermarket':
      getID('iphone_img').style.background = 'url(image/supermarket.png) 0 0/contain';
      addClass('active', getID('supermarket'));
      index = 3;
      break;
  }
}
...
carousel_part_2 = () => {
  ...
  // 获取上下箭头元素。
  var preArrow = getID('casePre');
  var nextArrow = getID('caseNext');
